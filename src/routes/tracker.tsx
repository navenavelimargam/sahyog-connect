...
function TrackerPage() {
  ...
  const submitRating = async () => {
    ...
    try {
      ...
      if (volunteerError || !volunteer) throw new Error("Could not load volunteer data");
      ...
      await supabase.from("notifications").insert({
        ...
        message: feedback?.length
          ? `"${feedback}" — Your new average rating is ${nextRating} ⭐`
          : `A community member rated your help ${rating}/5. Your new average rating is ${nextRating} ⭐. Keep up the great work!`,
        icon: "⭐",
      });
    } catch (error) {
      toast.error(error instanceof Error ? error.message : "Could not submit rating");
